                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{route('backend.index', 'ru')}}">{{__('locale.home')}}</a></li>
                ...
                <div class="row">
                    <div class="col-md-9">
                        <div class="action-content">
                            <button style="margin-bottom: 10px" class="btn btn-danger delete_all" data-url="{{ route('forthebuilder.action-logs.destroyMultiple') }}">{{__('locale.delete_selected')}}</button>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="create-data" style="float: right;">
                            <form style="display: inline-block;" action="{{route('forthebuilder.action-logs.destroyAll')}}" method="POST">
                                @csrf
                                @method('DELETE')
                ...
                @if(!empty($models))
                    @foreach($models as $key => $model)
                        <tr>
                            ...
                            <td>@if($model->user&&$model->user->first_name){{$model->user->first_name}}@endif</td>
                            <td>@if($model->user&&$model->user->last_name){{$model->user->last_name}}@endif</td>
                            <td>{{$model->message}}</td>
                            ...
                            <td>
                                <div style="text-align: center;">
                                    <a href="{{route('forthebuilder.action-logs.show',$model->id)}}" class="btn btn-primary" title="show"><i class="fas fa-eye"></i></a>
                                    <form style="display: inline-block;" action="{{route('forthebuilder.action-logs.destroy',$model->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
