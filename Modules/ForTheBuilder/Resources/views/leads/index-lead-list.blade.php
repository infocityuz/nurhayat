    ...
    <div class="card">
        <div class="content-header">
            <div class="container-fluid card-block">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">{{ translate('leads') }}</h1>
                    </div>
                    <div class="col-sm-6">
                    ...
                    @foreach ($leadStatuses as $leadstatus)
                        <div class="card card-row ">
                            ...
                            <div class="lead-list-create">
                                <a href="{{ route('forthebuilder.leads.create', ['status' => 1]) }}"
                                    class="lead-list-create-a">
                                    {{ translate('create lead') }}
                                    {{-- {{ __('locale.create') }} {{ __('locale.lead') }} --}}
                                </a>
                            </div>
                            <div class="card-body " id="{{ $leadstatus->id }}" ondrop="drop(event)"
                                ondragover="allowDrop(event)">
                                @foreach ($leadstatus->leads as $leads)
                                    <div class="items-1-id" id="{{ $leads->id }}" draggable="true"
                                        ...
                                        <div class="card card-light card-outline">
                                            <div class="card-header">
                                                <div class="">
                                                    <p>
                                                        <small>
                                                            ...
                                                            {{-- {{ __('locale.admin_id') . ': ' }} --}}
                                                            <b>
                                                                {{ $leads->users ? $leads->users->last_name . ' ' . $leads->users->first_name : translate('Not specified') }}
                                                            </b>
                                                        </small>
                                                    </p>
                                                    <h5 class="card-title lead-list-item-title">
                                                        <ion-icon name="person" style="color: #6c757d"></ion-icon>
                                                        <a href="{{ route('forthebuilder.leads.show', $leads->id) }}">{{ $leads->name }}
                                                            <br></a>
                                                ...
                                                <div>
                                                    <p>
                                                        <small
                                                            class="float-right">{{ translate('Date') . ': ' . date('d.m.Y', strtotime($leads->created_at)) }}
                                                        </small>
                                                    </p>
                                                    <br>
                                                    <p>
                                                        <small
                                                            class="float-right">{{ translate('Time') . ': ' . date('H:i:s', strtotime($leads->created_at)) }}
                                                        </small>
                                                    </p>
                                                </div>
                                            </div>
